...
//
// The world is defined by a pure function voxel_at(p, seed) -> u32 material
...
//
// World recipe (seeded, ~1-3% occupancy at 1024^3):
//   - terrain shell: y < height(x, z) with fbm-ish value noise, but only a
//     4-voxel-thick crust is solid (true sparse occupancy, like streamed
//     game worlds, instead of a half-full solid volume)
...
fn terrain_height(xz : vec2<u32>, seed : u32) -> f32 {
  let p = vec3<u32>(xz.x, 0u, xz.y);
  let h = vnoise(p, 256u, seed) * 0.55 + vnoise(p, 64u, seed + 1u) * 0.30 + vnoise(p, 16u, seed + 2u) * 0.15;
...
fn voxel_material(p : vec3<u32>, seed : u32) -> u32 {
  ...
  if ((sh & 31u) == 3u) {                      // ~3% of sites carry a tower
    ...
    let base = terrain_height(vec2<u32>(u32(cx), u32(cz)), seed);
    let top = base + 48.0 + f32((sh >> 8u) & 63u);
    if (abs(f32(p.x) - cx) <= half && abs(f32(p.z) - cz) <= half && fy > base - 4.0 && fy < top) {
  ...
  // Terrain crust: solid only within 4 voxels below the surface.
  if (fy < h && fy > h - 4.0) {
